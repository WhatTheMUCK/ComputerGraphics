...
vector <vector <vec3>> build_figure(double a, double b, double da, double height)
{
    ...
    for (double theta = 0; theta < 2 * pi; theta += da)
    {
        bot.push_back(get_cylinder_point(theta, -height / 2, a, b));
        top.push_back(get_cylinder_point(theta, height / 2, a, b));
    }
    reverse(top.begin(), top.end());
    figure.push_back(top);
    figure.push_back(bot);
    return figure;
}
...
void draw_figure(vector <vector <vec3>>& figure, bool inside)
{
    ...
    {
        float red = (sin(time) + 1) / 2; // Значение от 0 до 1
        float green = (sin(time + pi / 2) + 1) / 2; // Смещение фазы на 90 градусов
        float blue = (sin(time + pi) + 1) / 2; // Смещение фазы на 180 градусов
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(900, 500, "LR6", nullptr, nullptr);
    if (!window)
    {
        glfwTerminate();
        return -1;
